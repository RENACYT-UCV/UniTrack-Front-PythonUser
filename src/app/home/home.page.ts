    ...
    if (currentUser && currentUser.idUsuario) {
      ...
      this.blockchainService.addBlock(data, currentUser.idUsuario).subscribe({
        next: async (response) => {
          ...
          const qrHash = `${currentUser.idUsuario}-${Date.now()}`;
          const tipoQr = this.isEntrada ? 'entrada' : 'salida';
          console.log(
            'Generating QR code with hash:',
            qrHash,
            'and type:',
            tipoQr
          );
          this.qrService.generateQrCode(qrHash, tipoQr).subscribe({
            next: (qrResponse) => {
              console.log('QR Code generated by backend:', qrResponse);
              // After generating the QR, register it in the backend with the type
              this.qrService
                .registrarQr(
                  qrHash,
                ...
                .subscribe({
                  next: (registroResponse) => {
                    console.log('QR registered in backend:', registroResponse);
                    this.loadQrCodeForUser(currentUser.idUsuario);
                  },
                  error: (registroError) => {
                    console.error('Error registering QR:', registroError);
                  },
                });
            },
            error: (qrError) => {
